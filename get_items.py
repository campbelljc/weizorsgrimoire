...
def get_items_from_summit():
    ...
    for link in links:
        ...
        if 'sets' in link:
            set_names = tree.xpath('//tr/td/font/span/p/comment()')[1:-1]
            item_tier = ''
            item_type = ''
            quality = 'Set'
        ...
        if 'sets' in link:
            types = [typ for typ in tree.xpath('//tr/td//font//center//span/text()[normalize-space()]') if len(typ) > 0]
            print(types)
        else:
        ...
        assert len(names) > 0
        assert len(names) == len(types) and len(types) == len(descs) and len(descs) == len(images)

        for iname, itype, idesc, image in zip(names, types, descs, images):
            ...
            items.append(Item(iname.text_content(), 'http://classic.battle.net'+image, quality, item_tier, item_type, itype, attr_dict))
